...
vi.mock("@src/clients/seasun/cbjq/program-patch-game", () => ({
...
import { launchGameProgram } from "@src/clients/seasun/cbjq/program-launch-game";
import {
  patchProgram,
  patchRevertProgram,
} from "@src/clients/seasun/cbjq/program-patch-game";
...
describe("cbjq game process lifecycle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("monitors the configured nested executable and preserves launch settings", async () => {
    const { wine, raw, monitor } = createWine();
    const executable = "Game/Binaries/Win64/Game.exe";
    ...
    expect(writeFile).toHaveBeenCalledWith(
      ...
      expect.stringContaining(
        '"Z:\\game\\Game\\Binaries\\Win64\\Game.exe" -FeatureLevelES31 -ChannelID=42'
      )
    );
  ...
  it("rejects a duplicate before changing Wine or applying patches", async () => {
  ...
  it("waits indefinitely for the prefix when process enumeration is unknown", async () => {
  ...
  it("kills the prefix before restoring after a startup timeout", async () => {
