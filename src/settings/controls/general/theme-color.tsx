...
const CONFIG_KEY = configEntries.themeColor.key;
...
export default async function createThemeColorConfig({
  ...
  try {
    config.themeColor =
      (await store.read(configEntries.themeColor)) || DEFAULT_COLOR;
  } catch {
  ...
  async function onSave(save: boolean) {
    ...
    await store.write(configEntries.themeColor, config.themeColor);
