import { FormControl, FormLabel, HStack, Text } from "@hope-ui/solid";
...
export default async function ({
  ...
  try {
    stored = Math.round(
      parseFloat((await store.read(configEntries.preferredMaxFps)) ?? "")
    );
  } catch {
  ...
  async function onSave(apply: boolean) {
    ...
    await store.write(configEntries.preferredMaxFps, value());
  }
    ...
    function UI() {
      return (
        <FormControl>
          ...
          <Text size="xs" color="$neutral11" mt="$1" userSelect="none">
            {locale.get("SETTING_PREFERRED_MAX_FPS_DESC")}
          </Text>
        </FormControl>
      );
    },
